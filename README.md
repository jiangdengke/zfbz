...
打开 <http://127.0.0.1:4173>，先输入 Web 访问密码，再输入壁纸 ID。页面会先回显公开缩略图供确认，点击“开始下载”只下载原图；任务在服务端临时中转，完成后自动保存到浏览器下载目录，并清理服务器临时文件。代理由服务端的 `.env` / `.env.haowallpaper` 统一配置，前端不接收或保存代理地址和凭据。
